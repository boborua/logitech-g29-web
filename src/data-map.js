...
//----------
/*
Details on each item of the read buffer provided by WebHID for the Logitech G29.
...
function dataMap(dataDiffPositions, data, memory) {
  /*
    ...
    @param   {Buffer}  data               Buffer data from a WebHID event.
...
function wheelShiftPedals(data, memory) {
  ...
  memory.wheel.shift_right = d & 1 ? 1 : 0;
...
function wheelSpinnerAndButtons(data, memory) {
  ...
  memory.wheel.button_minus = d & 1 ? 1 : 0;
  memory.wheel.spinner = d & 2 ? 1 : 0;
  memory.wheel.spinner = d & 4 ? 2 : memory.wheel.spinner;
...
function pedalToPercent(num) {
  /*
    Convert a pedal value to a percent.
    @param   {Number}  num  Number between 0 and 255.
    @return  {Number}  num  Number between 0 and 100.
    */
  num = 255 - num;

  num = round(num / 2.55, 0);
